 ...
 */
export function createTensor(name, indices, shape, initializer = 'zeros') {
 ...
 */
export function fromMatrix(name, indices, matrix) {
 ...
 */
export function fromVector(name, index, values) {
    return {
        name,
        shape: [values.length],
        indices: [index],
 ...
 */
export function getElement(tensor, ...indices) {
    let flatIndex = 0;
    let stride = 1;
 ...
 */
export function setElement(tensor, value, ...indices) {
    let flatIndex = 0;
    let stride = 1;
    for (let i = tensor.shape.length - 1; i >= 0; i--) {
        flatIndex += indices[i] * stride;
 ...
 */
export function einsum(notation, ...tensors) {
 ...
 */
export function threshold(tensor, t = 0) {
 ...
 */
export function sigmoid(tensor, temperature = 1) {
    const data = new Float64Array(tensor.data.length);
    for (let i = 0; i < tensor.data.length; i++) {
        if (temperature === 0) {
 ...
 */
export function softmax(tensor, axis = -1) {
 ...
 */
export function multiply(...tensors) {
    const first = tensors[0];
    const data = new Float64Array(first.data.length);
    for (let i = 0; i < data.length; i++) {
 ...
 */
export function transpose(tensor) {
 ...
 */
export function tensorToString(tensor, precision = 3) {
    if (tensor.shape.length === 1) {
 ...
 */
export function identity(name, indices, size) {
