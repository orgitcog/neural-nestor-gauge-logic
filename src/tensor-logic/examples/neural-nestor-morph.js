 ...
 */
import { fromVector, tensorToString, } from '../core';
import { createNestor, createFiberBundle, attachFiberBundle, traverseNestor, nestorDepth, nestorNodeCount, } from '../nestor';
import { createFiberForest, embedNestorInForest, createTypedHyperGraphNN, neuralNestorMorphForward, addObject, createCategoricalContext, } from '../neural-nestor-morph';
/**
 * Run the Neural Nestor Morph Logic example.
 */
export function runNeuralNestorMorphExample() {
    const steps = [];
...
This enables typed reasoning over the hyper-graph structure.`,
    ...
    // People
    const alice = createNestor('alice', fromVector('alice_features', 'p', [0.8, 0.9, 0.7]), personType);
    const bob = createNestor('bob', fromVector('bob_features', 'p', [0.6, 0.8, 0.9]), personType);
    const charlie = createNestor('charlie', fromVector('charlie_features', 'p', [0.9, 0.7, 0.8]), personType);
    // Team with people as children
    const aiTeam = createNestor('ai_team', fromVector('ai_team_features', 't', [0.9, 0.8, 0.85, 0.9, 0.88]), teamType, [alice, bob]);
    const dataTeam = createNestor('data_team', fromVector('data_team_features', 't', [0.85, 0.9, 0.8, 0.87, 0.9]), teamType, [charlie]);
    steps.push({
...
- Children (forming the hypertree structure)`,
    ...
    const engineeringDept = createNestor('engineering', fromVector('eng_dept_features', 'd', [0.9, 0.85, 0.9, 0.88, 0.87, 0.9, 0.86]), deptType, [aiTeam, dataTeam]);
    // Organization with department as child
    const techCorp = createNestor('techcorp', fromVector('techcorp_features', 'o', [0.95, 0.9, 0.88, 0.92, 0.89, 0.91, 0.9, 0.87, 0.93, 0.88]), orgType, [engineeringDept]);
    steps.push({
...
This forms a rooted hypertree with 6 nodes across 4 levels.`,
    ...
    const aiTeamWithFiber = attachFiberBundle({ ...aiTeam, children: [aliceWithFiber, bobWithFiber] }, aiTeamFiber);
    const dataTeamWithFiber = attachFiberBundle({ ...dataTeam, children: [charlieWithFiber] }, dataTeamFiber);
    const engWithFiber = attachFiberBundle({ ...engineeringDept, children: [aiTeamWithFiber, dataTeamWithFiber] }, engFiber);
    const techCorpWithFiber = attachFiberBundle({ ...techCorp, children: [engWithFiber] }, techCorpFiber);
    steps.push({
...
Each fiber has the same dimension as the node's feature space.`,
    ...
    const typeSystem = new Map([
        ['Person', personType],
        ['Team', teamType],
        ['Department', deptType],
        ['Organization', orgType],
    ]);
    const forest = createFiberForest([techCorpWithFiber], embeddingDim, typeSystem);
    steps.push({
