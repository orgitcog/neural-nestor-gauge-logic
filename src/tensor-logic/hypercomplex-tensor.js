 ...
 */
import { AlgebraType, Complex, Quaternion, Octonion, Sedenion, CayleyDickson, algebraDimension, } from './hypercomplex';
/**
 * Create a hypercomplex tensor
 */
export function createHypercomplexTensor(name, indices, shape, algebraType, initializer = 'zeros') {
    ...
    if (Array.isArray(initializer)) {
        ...
        data = Array.from({ length: size }, () => new CayleyDickson(new Float64Array(dim), algebraType));
    }
 ...
 */
export function createComplexTensor(name, indices, shape, initializer = 'zeros') {
    const size = shape.reduce((a, b) => a * b, 1);
    let data;
    if (Array.isArray(initializer)) {
        data = initializer;
        ...
        data = Array.from({ length: size }, () => new Complex(Math.random() - 0.5, Math.random() - 0.5));
    }
 ...
 */
export function createQuaternionTensor(name, indices, shape, initializer = 'zeros') {
    const size = shape.reduce((a, b) => a * b, 1);
    let data;
    if (Array.isArray(initializer)) {
        data = initializer;
        ...
        data = Array.from({ length: size }, () => new Quaternion(Math.random() - 0.5, Math.random() - 0.5, Math.random() - 0.5, Math.random() - 0.5));
    }
 ...
 */
export function getHypercomplexElement(tensor, ...indices) {
    let flatIndex = 0;
    let stride = 1;
 ...
 */
export function setHypercomplexElement(tensor, value, ...indices) {
    let flatIndex = 0;
    let stride = 1;
    for (let i = tensor.shape.length - 1; i >= 0; i--) {
        flatIndex += indices[i] * stride;
 ...
 */
export function hypercomplexEinsum(notation, ...tensors) {
    ...
    const dim = algebraDimension(algebraType);
    const outputData = Array.from({ length: outputSize }, () => {
        const components = new Float64Array(dim);
        return new CayleyDickson(components, algebraType);
    });
 ...
 */
export function mapHypercomplexTensor(tensor, fn) {
    return {
        ...tensor,
 ...
 */
export function splitActivation(tensor, fn) {
 ...
 */
export function modulusActivation(tensor, fn) {
    return mapHypercomplexTensor(tensor, (value) => {
 ...
 */
export function complexReLU(tensor) {
 ...
 */
export function quaternionReLU(tensor) {
 ...
 */
export function hypercomplexTensorToString(tensor, precision = 3) {
