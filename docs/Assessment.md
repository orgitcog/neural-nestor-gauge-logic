...
So to fix this I sat down and gave Prof. Domingos' paper a good, old-fashioned read with yellow high-lighter and a fine, blue gel-point pen, and a flat metal ruler, initially focusing on the juicy hunks of Tensor Logic code Tables 1, 2 and 3, but also collecting about a couple of dozen or so other tid-bits of concepts and methods that contained small examples of Tensor Logic formulae. Then I asked the chatbot in the retrospective chat to slowly build up, example by example, by reading that paper and put its findings for each of those items I called out and then putting the results in [Paper-examples.md](Paper-examples.md). I could immediately tell it was on the right track. 
- This not only nailed down the precious notation but also curated many different examples. Thus, I was able to use that as a kind of RAG basis to get the chatbot to fix the examples in the web app so they looked like Prof. Domingos could have written them. This really "saved the day" for the user-oriented content (the actual purpose of and point to, the web site), without which the web site would have had excellent infrastructure but truly crappy content.
- Of course, further verification work still needs to be done to be sure the examples are truly correct!
