...
- This not only nailed down the precious notation but also curated many different examples. Thus, I was able to use that as a kind of RAG basis to get the chatbot to fix the examples in the web app so they looked like Prof. Domingos could have written them. This really "saved the day" for the user-oriented content (the actual purpose and point of, the web site), without which the web site would have had excellent infrastructure but truly crappy content.
